...
cd /home/pwdemo.stefan/pw/jobs/organic-javelin/logs/setup-and-launch/step_3
echo $$ > step.pid
...
echo Run git fetch
git fetch origin June-2023-log10-gss 2>/dev/null || true
if git ls-remote --exit-code --heads origin June-2023-log10-gss >/dev/null 2>&1; then
  # exists remotely: check it out tracking origin, fast-forward to latest
  git checkout -B "June-2023-log10-gss" --track "origin/June-2023-log10-gss"
else
  # doesn't exist: create it fresh from current main
  git checkout -b "June-2023-log10-gss" main
fi
git pull
cd ${HOME}
