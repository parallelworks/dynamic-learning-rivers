...
# I live in /core/pworks-main/swift-pw-bin/parslab/parsl-templates
...
try:
    ...
    #print('\npwprops:')
    #print(pwprops, flush = True)
    
    os.environ['PW_PARSL_WORKER_HOME'] = pwprops['workDirectory']
    os.environ['keepSiteDir'] = pwprops['keepSiteDir']
    
    # Parsl retries: (AV 03-05-2021)
    try:
        executionRetries = int(pwprops['executionRetries'])
    except:
        print('WARNING: Could not read executionRetries!', flush = True)
        print('WARNING: executionRetries: {}'.format(pwprops['executionRetries']), flush = True)
        executionRetries = 0
